...
#include "network_socket_interface_unix.h"
...
UDPSocketInterfaceUnix::UDPSocketInterfaceUnix() : recv_thread_(nullptr),
  recv_count_(0),
  com_sockfd_(-1),
  ncd_(NET_NULL),
...
UDPSocketInterfaceUnix::~UDPSocketInterfaceUnix() {
  CloseSocket();
}

bool UDPSocketInterfaceUnix::CreateSocket(NetCommDevTypeDef obj, const char *ip, const char *port) {
...
bool UDPSocketInterfaceUnix::CloseSocket() {
...
bool UDPSocketInterfaceUnix::RecvFromNet(uint8_t *rx_buf , uint32_t rx_buff_len, uint32_t *rx_len) {
...
bool UDPSocketInterfaceUnix::TransToNet(uint8_t *tx_buf, uint32_t tx_buff_len, uint32_t *tx_len) {
  int32_t len = -1;
...
void UDPSocketInterfaceUnix::SetRecvCallback(std::function<void(const char *, size_t length)> callback) {
    recv_callback_ = callback;
}  

void UDPSocketInterfaceUnix::RecvThreadProc(void *param) {
  UDPSocketInterfaceUnix *cmd_if = (UDPSocketInterfaceUnix *)param;
...
TCPSocketInterfaceUnix::TCPSocketInterfaceUnix() : recv_thread_(nullptr),
  recv_count_(0),
  com_sockfd_(-1),
...
TCPSocketInterfaceUnix::~TCPSocketInterfaceUnix() {
  CloseSocket();
}
  
bool TCPSocketInterfaceUnix::CreateSocket(NetCommDevTypeDef obj, const char *ip, const char *port) {
  if (is_cmd_created_ == true) {
    return true;
  }
...
bool TCPSocketInterfaceUnix::CloseSocket() {
  if (is_cmd_created_ == false) {
    return true;
  }
...
bool TCPSocketInterfaceUnix::RecvFromNet(uint8_t *rx_buf , uint32_t rx_buff_len, uint32_t *rx_len) {
  static timespec timeout = {0, (long)(100 * 1e6)};
  int32_t len = -1;
...
bool TCPSocketInterfaceUnix::TransToNet(uint8_t *tx_buf, uint32_t tx_buff_len, uint32_t *tx_len) {
  int32_t len = 1;
...
void TCPSocketInterfaceUnix::SetRecvCallback(std::function<void(const char *, size_t length)> callback) {
  recv_callback_ = callback;
}

void TCPSocketInterfaceUnix::RecvThreadProc(void *param) {
  TCPSocketInterfaceUnix *cmd_if = (TCPSocketInterfaceUnix *)param;
  char *recieve_buff = new char[MAX_RECV_BUF_LEN + 1];
