...
LDLidarDriverUnixInterface::LDLidarDriverUnixInterface() :  comm_pkg_(new LdLidarDataProcess()),
  comm_serial_(new SerialInterfaceUnix()),
  comm_tcp_network_(new TCPSocketInterfaceUnix()),
  comm_udp_network_(new UDPSocketInterfaceUnix()){
  
  last_pubdata_times_ = std::chrono::steady_clock::now();
}

LDLidarDriverUnixInterface::~LDLidarDriverUnixInterface() {
...
bool LDLidarDriverUnixInterface::Connect(LDType product_name, 
...
bool LDLidarDriverUnixInterface::Disconnect(void)  {
  if (!is_connect_flag_) {
    return true;
  }
...
void LDLidarDriverUnixInterface::EnablePointCloudDataFilter(bool is_enable) {
  comm_pkg_->SetNoiseFilter(is_enable);
}

bool LDLidarDriverUnixInterface::WaitLidarComm(int64_t timeout) {
...
LidarStatus LDLidarDriverUnixInterface::GetLaserScanData(Points2D& dst, int64_t timeout) {
  if (!is_start_flag_) {
...
LidarStatus LDLidarDriverUnixInterface::GetLaserScanData(LaserScan& dst, int64_t timeout) {
  if (!is_start_flag_) {
    return LidarStatus::STOP;
  }
...
bool  LDLidarDriverUnixInterface::GetLidarScanFreq(double& spin_hz) {
  if (!is_start_flag_) {
    return false;
  }
...
void LDLidarDriverUnixInterface::RegisterGetTimestampFunctional(std::function<uint64_t(void)> get_timestamp_handle) {
  register_get_timestamp_handle_ = get_timestamp_handle;
}

uint8_t LDLidarDriverUnixInterface::GetLidarErrorCode(void) {
  if (!is_start_flag_) {
...
bool LDLidarDriverUnixInterface::Start(void) {
  if (is_start_flag_) {
    return true;
  }
...
bool LDLidarDriverUnixInterface::Stop(void) {
  if (!is_start_flag_) {
    return true;
  }
